...
if(!isset($params["what"])){
	...
	$tmpid = $db->GenID(cms_db_prefix()."module_avplayersaved_queries_seq");
	$name = isset($params["queryname"])?addslashes($params["queryname"]):"";
	
	$query = "INSERT INTO ".cms_db_prefix()."module_avplayer_saved_queries SET id=?, name=?, what=?, whereclause=?, wherevalues=?, queryorder=?";
