...
if (isset($params["submit"]) || 
	isset($params["apply"]) ||
	isset($params["add_flvfile"]) ||
	isset($params["remove_flvfile"]) ||
	isset($params["add_mp4file"]) ||
	isset($params["remove_mp4file"]) ||
	isset($params["add_webmfile"]) ||
	isset($params["remove_webmfile"]) ||
	isset($params["add_ogvfile"]) ||
	isset($params["remove_ogvfile"]) ||
	isset($params["add_poster"]) ||
	isset($params["remove_poster"]) ||
	isset($params["add_mp3file"]) ||
	isset($params["remove_mp3file"]) 
	)
{
	...
	if(isset($params["oldparent"]) && $params["oldparent"] != $item->parent){
		...
		// REDIRECTING...
			if(isset($params["add_flvfile"])){
				$params = array("Bid"=>$item->id,"field"=>"flvfile","tablename"=>"avplayer_mediafile","prefix"=>"B", "startdir"=>"media", "extensions"=>"flv", "mode"=>"file");
				$this->Redirect($id, "browsefiles", $returnid, $params);
			}elseif(isset($params["remove_flvfile"])){
				$params = array("Bid"=>$item->id,"field"=>"flvfile","level"=>"mediafile","tablename"=>"avplayer_mediafile","prefix"=>"B");
				$this->Redirect($id, "assignfile", $returnid, $params);
			}elseif(isset($params["add_mp4file"])){
				...
				$this->Redirect($id, "assignfile", $returnid, $params);
			}elseif(isset($params["add_webmfile"])){
				$params = array("Bid"=>$item->id,"field"=>"webmfile","tablename"=>"avplayer_mediafile","prefix"=>"B", "startdir"=>"media", "extensions"=>"webm", "mode"=>"file");
				$this->Redirect($id, "browsefiles", $returnid, $params);
			}elseif(isset($params["remove_webmfile"])){
				$params = array("Bid"=>$item->id,"field"=>"webmfile","level"=>"mediafile","tablename"=>"avplayer_mediafile","prefix"=>"B");
				$this->Redirect($id, "assignfile", $returnid, $params);
			}elseif(isset($params["add_ogvfile"])){
				$params = array("Bid"=>$item->id,"field"=>"ogvfile","tablename"=>"avplayer_mediafile","prefix"=>"B", "startdir"=>"media", "extensions"=>"ogv", "mode"=>"file");
				$this->Redirect($id, "browsefiles", $returnid, $params);
			}elseif(isset($params["remove_ogvfile"])){
				$params = array("Bid"=>$item->id,"field"=>"ogvfile","level"=>"mediafile","tablename"=>"avplayer_mediafile","prefix"=>"B");
				$this->Redirect($id, "assignfile", $returnid, $params);
				...
				$this->Redirect($id, "assignfile", $returnid, $params);
			}elseif(isset($params["add_mp3file"])){
				$params = array("Bid"=>$item->id,"field"=>"mp3file","tablename"=>"avplayer_mediafile","prefix"=>"B", "startdir"=>"media", "extensions"=>"mp3", "mode"=>"file");
				$this->Redirect($id, "browsefiles", $returnid, $params);
			}elseif(isset($params["remove_mp3file"])){
				$params = array("Bid"=>$item->id,"field"=>"mp3file","level"=>"mediafile","tablename"=>"avplayer_mediafile","prefix"=>"B");
				$this->Redirect($id, "assignfile", $returnid, $params);
...
$this->smarty->assign("flvfile_input", ((isset($item->flvfile) && $item->flvfile)?$item->flvfile->pic.'<a href="'.$item->flvfile->url.'" >'.$item->flvfile->filepath."</a> (".$item->flvfile->size_wformat.")<br/>".$this->CreateInputSubmit($id,"remove_flvfile",$this->Lang("Remove"))." ":"").$this->CreateInputSubmit($id,"add_flvfile",$this->Lang("mediafile_select_flvfile")));
...
$this->smarty->assign("webmfile_input", ((isset($item->webmfile) && $item->webmfile)?$item->webmfile->pic.'<a href="'.$item->webmfile->url.'" >'.$item->webmfile->filepath."</a> (".$item->webmfile->size_wformat.")<br/>".$this->CreateInputSubmit($id,"remove_webmfile",$this->Lang("Remove"))." ":"").$this->CreateInputSubmit($id,"add_webmfile",$this->Lang("mediafile_select_webmfile")));
$this->smarty->assign("ogvfile_label", $this->Lang("mediafile_ogvfile"));
$this->smarty->assign("ogvfile_input", ((isset($item->ogvfile) && $item->ogvfile)?$item->ogvfile->pic.'<a href="'.$item->ogvfile->url.'" >'.$item->ogvfile->filepath."</a> (".$item->ogvfile->size_wformat.")<br/>".$this->CreateInputSubmit($id,"remove_ogvfile",$this->Lang("Remove"))." ":"").$this->CreateInputSubmit($id,"add_ogvfile",$this->Lang("mediafile_select_ogvfile")));
...
$this->smarty->assign("mp3file_input", ((isset($item->mp3file) && $item->mp3file)?$item->mp3file->pic.'<a href="'.$item->mp3file->url.'" >'.$item->mp3file->filepath."</a> (".$item->mp3file->size_wformat.")<br/>".$this->CreateInputSubmit($id,"remove_mp3file",$this->Lang("Remove"))." ":"").$this->CreateInputSubmit($id,"add_mp3file",$this->Lang("mediafile_select_mp3file")));
