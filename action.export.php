...
";
	...
	// exporting multiple file fields (if any)
		$tmpoutput = '';
		$query = "SELECT * FROM ".cms_db_prefix()."module_".$this->GetName()."_multiplefilesfields";
		$dbresult = $db->Execute($query);
		while ( $dbresult && $row = $dbresult->FetchRow() ) {
			$tmpoutput .= '<item>
		';
			foreach($row as $field=>$value){
				$tmpoutput .= '<'.$field.'><![CDATA['.stripslashes($value).']]></'.$field.'>
		';
			}
			$tmpoutput .= '
	</item>
	';
		...
		if($tmpoutput != '')	$output .= '
<table name="multiplefilesfields">
	'.$tmpoutput.'
</table>
...
';
	...
	// exporting listoptions (if any)
		$tmpoutput = '';
		$query = "SELECT * FROM ".cms_db_prefix()."module_".$this->GetName()."_fieldoptions";
		$dbresult = $db->Execute($query);
		while ( $dbresult && $row = $dbresult->FetchRow() ) {
			$tmpoutput .= '<item>
		';
