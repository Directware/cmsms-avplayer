	...
	case "delete":
		...
		if(isset($params["sharedbyparents"]) && $params["sharedbyparents"]){
			...
			if($parentlevel = $this->get_nextlevel($params["levelname"], false)){
				$linktable = cms_db_prefix()."module_avplayer_".$parentlevel."_has_".$params["levelname"];
				$query = "DELETE FROM $linktable WHERE ".$params["levelname"]."_id=?";
				$db->Execute($query, array($itemid));
			}
		}
		...
		if(isset($params["addfiles"]) && $params["addfiles"] != ""){
			...
			if(count($fields) > 0){
				...
				$addfilestable = cms_db_prefix()."module_avplayer_multiplefilesfields";
				...
				if($this->GetPreference("delete_files",false)){
					...
					while($dbresult && $row = $dbresult->FetchRow()){
						$filepath = $gCms->config["uploads_path"].(substr($row["filepath"],0,1)=="/"?"":"/").$row["filepath"];
						if(file_exists($filepath))	unlink($filepath);
					}
				}
		...
		if(isset($params["levelname"]))	$this->SendEvent("avplayer_deleted", array("what"=>$params["levelname"]));
