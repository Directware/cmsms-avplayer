...
$mediaext = array('swf','flv','jpeg','jpg','png','gif','mp4');
...
function imagemode_buildlinkcontent($baseurl, $filename, $dir=''){
	...
	if(in_array($extension, array('flv','swf','wmv','mov'))){
		$pic = $admintheme->DisplayImage("icons/filetypes/fmedia.gif");
	}elseif(in_array($extension, array('jpeg','jpg','png','gif'))){
		$pic = $admintheme->DisplayImage("icons/filetypes/fpaint.gif");
	}else{
		$pic = $admintheme->DisplayImage("icons/filetypes/fdoc.gif");
	}
...
function makeDirObj($filename,$dir,$module,$params,$curdir,$parentdir,$id,$returnid){
	...
	$obj->pic = $admintheme->DisplayImage("icons/filetypes/".($parent?"up":"")."folder.gif");
	if($parent){
...
function makeFileObj($filename,$dir,$module,$params,$curdir,$id,$returnid,$baseurl){
	...
	$obj->pic = $admintheme->DisplayImage($info["fileicon"]);
	$obj->imagesize = '';
	$obj->viewlink = '';
	if($info["imagesize"]){
		$obj->imagesize = '<a style="display:none;">'.$info["width"].'</a>'.$info["imagesize"];
		$thethumb = get_thumbnail($baseurl, $filename, $dir, true);
		if($thethumb)	$obj->viewlink = ' <a href="'.$thethumb[0].'" title="'.$module->Lang("instantpreview").'" style="cursor: pointer;" onclick="return displaythumb(this,\''.$thethumb[0].'\','.$thethumb[1].','.$thethumb[2].');">'.$admintheme->DisplayImage("icons/system/view.gif").'</a>';
	}
	$obj->deletelink = $module->CreateLink($id, 'browsefiles', $returnid, $admintheme->DisplayImage("icons/system/delete.gif"),array_merge($params, array('deletefile'=>$filename))).$obj->viewlink;
...
';
	if($upfolder){
		echo $divoption;
		$newparams = array_merge($params, array('curdir' => $parentdir));
		echo $this->CreateLink($id, 'browsefiles', $returnid, $admintheme->DisplayImage("icons/filetypes/upfolder.gif").'<br/>'.$this->Lang('parentdir'), $newparams);
		echo $closediv;
	}
	...
	foreach($files as $filename){
		...
		echo $this->CreateLink($id, 'browsefiles', $returnid, $admintheme->DisplayImage("icons/system/delete.gif"),$newparams);
		echo $closediv;
	}
	echo '</div></div>';
